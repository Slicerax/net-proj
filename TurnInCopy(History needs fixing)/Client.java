...
public class Client
{
	...
	*/
	public static void main(String[] args)
	{
		...
		str = input.nextLine();
		if(!(str.startsWith("Log on") || str.startsWith("LOG ON") || str.startsWith("log on")))
		{
		...
		try
		{
			...
			while(true){
				...
				System.out.println("Chat <Client-ID>");
				System.out.println("History <Client-ID>");
				System.out.println("Available (Typing Available will make you available to receive chat requests)"); //to help waiting problem, if online, people can contact, can wait for TCP connection stream

				Scanner userIn = new Scanner(System.in);
				String choice = userIn.nextLine();
				String choiceTokens[] = choice.split(" ");
				if (choice.equals("Log off")){
					messageOut = "LOG_OFF";
					messageOut = prepareOutMessage(data, messageOut, clientIndex, encryptKeys);
					out.println(messageOut);
					clientTCP.close();
				...
				else if(choiceTokens[0].equals("Chat")){
					//chat with user, send chat request via TCP connection
					System.out.println(choiceTokens[1]);
					messageOut = "CHAT_REQUEST " + choiceTokens[1] + " " + randCookie;
					messageOut = prepareOutMessage(data,messageOut, clientIndex, encryptKeys);
				...
				else if(choiceTokens[0].equals("History")){
					messageOut = "HISTORY_REQ " + choiceTokens[1];
					messageOut = prepareOutMessage(data, messageOut, clientIndex, encryptKeys);
					out.println(messageOut);
